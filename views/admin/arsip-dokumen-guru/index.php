...
/** @var app\models\ArsipDokumenGuruSearch $searchModel */
/** @var yii\data\ActiveDataProvider $dataProvider */

$this->title = 'Arsip Dokumen Guru';
$this->params['breadcrumbs'][] = $this->title;

?>
<div class="arsip-dokumen-guru-index">

    <div class="card">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                <h1 class="h3 m-0"><?= Html::encode($this->title) ?></h1>
                <div class="person-sub">Kelola arsip dokumen guru</div>
            </div>
            <div>
        ...
        <div class="table-wrap">
            <div style="display:flex;gap:12px;align-items:center;margin-bottom:12px">
                <input id="arsip-guru-search" class="top-search" placeholder="Cari dokumen atau nomor..." style="flex:1" />
                <select id="arsip-guru-kategori" class="top-search" style="width:210px;padding:8px 10px">
            ...
            <?= GridView::widget([
                ...
                'columns' => [
                    [
                        ...
                        'format' => 'raw',
                        'value' => function($m){
                            $code = isset($m->no_dokumen) ? Html::encode($m->no_dokumen) : 'DG-'.Html::encode($m->id_arsip_guru);
                    ...
                    [
                        ...
                        'value' => function($m){
                            if(isset($m->guru) && isset($m->guru->nama_guru)) return Html::encode($m->guru->nama_guru);
                            return Html::encode($m->id_guru);
                        }
                    ],

                    [
                        'label' => 'NIP',
                        'value' => function($m){ return isset($m->guru->nip) ? $m->guru->nip : ''; }
                    ],
                    ...
                    [
                        ...
                        'value' => function($m){
                            if(isset($m->file_path) && !empty($m->file_path)){
                                ...
                                $viewUrl = \yii\helpers\Url::to(['file/view-guru', 'filename' => basename($m->file_path)]);
                                $downloadUrl = \yii\helpers\Url::to(['file/download-guru', 'filename' => basename($m->file_path)]);
                    ...
                    [
                        ...
                        'urlCreator' => function ($action, ArsipDokumenGuru $model, $key, $index, $column) {
                            return Url::toRoute([$action, 'id_arsip_guru' => $model->id_arsip_guru]);
                        }
                    ],
                ],
            ]); ?>
...
(function(){
  const input = document.getElementById('arsip-guru-search');
  const select = document.getElementById('arsip-guru-kategori');
  const table = document.querySelector('.arsip-dokumen-guru-index .table-wrap table');
  if(!table) return;
  const rows = Array.from(table.tBodies[0].rows);
  // populate categories from table
  const cats = new Set();
  rows.forEach(r=>{
    const catCell = r.querySelector('td:nth-child(4)');
    if(catCell){ cats.add(catCell.innerText.trim()); }
  });
  cats.forEach(c=>{ if(c!==''){
  ...
  function filter(){
    ...
    rows.forEach(r=>{
      ...
      const catText = (r.cells[3] && r.cells[3].innerText.trim())||'';
